    ...
    useEffect(() => {
        if(searchParam !== '' && searchParam.length >= 3 && goods) {
            ...
            goods.map(good => {
                ...
                let itemByCategory = good.category.toString().toLowerCase();

                // array of goods what starts with input
                item.startsWith(searchParam) && handleFillingArray(startSortedArray, good)

                // add sorting by category for goods
                itemByCategory === searchParam && handleFillingArray(sortedByCategoryArray, good)

                // array of goods what includes input
                item.includes(searchParam) && !item.startsWith(searchParam) && itemByCategory !== searchParam && handleFillingArray(endSortedArray, good)
            })

            // no goods found
            if(startSortedArray.length === 0 && endSortedArray.length === 0 && sortedByCategoryArray.length === 0) {
                setEmptyFilterArray(true)
                setFilterArray([])
            } 

            // join arrays
            setFilterArray(startSortedArray.filter((item, ind) => startSortedArray.indexOf(item) === ind).concat(endSortedArray.filter((item, ind) => endSortedArray.indexOf(item) === ind), sortedByCategoryArray.filter((item, ind) => sortedByCategoryArray.indexOf(item) === ind)))
